...
public class EdgeConfig implements Serializable {
     ...
     * an unbounded buffer, but with a limit which should be respected by a
     * well-behaving processor. When its outbox reaches the limit, the
     * processor should refrain from emitting any more items and yield control
     * back to its caller. This method sets the value of the limit.
     * The default value is {@value #DEFAULT_OUTBOX_LIMIT}.
     */
    public EdgeConfig setOutboxLimit(int outboxLimit) {
        this.outboxLimit = outboxLimit;
        return this;
    }

    /**
     * Returns the {@link #setOutboxLimit(int) limit} that will be
